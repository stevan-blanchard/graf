...
/*
 Licence : CeCILL_V2.1
 Auteur(s) : Tourneux Cédric(auteur de original) Blanchard Stevan (Modification)
*/

        $authorid = $dbh->prepare("SELECT id FROM author WHERE name = '".$_SESSION['authorName']."'");
        $authorid->execute();
        $id = $authorid->fetch();
    
        $displayRaf = $dbh->prepare("SELECT * FROM RAF WHERE author_id = ".$id['0']);
        $displayRaf->execute();
